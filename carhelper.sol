...
contract CarHelper is CarOvertaking{
    ...
    function setOvertakeFee(uint _fee){
        overtakeFee = _fee;
    }
    ...
    function modifyName(uint _id, string calldata _Name) external ratingRequire(3, _id) ownerOf(_id){
    cars[_Id].name = _Name;
    }
    function modifyVin(uint _id, uint _Vin) external ratingRequire(20, _id) ownerOf(_id) {
    cars[_id].vin = _Vin;
    }
